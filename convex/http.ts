...
http.route({
  ...
  handler: httpAction(async (ctx, request) => {
    ...
    if (!signature || !secretKey) {
      console.warn("Webhook received without signature or secret key");
    ...
    if (!isValid) {
      console.warn("Invalid signature in NabooPay webhook");
    ...
    try {
      ...
      console.log(`Processing NabooPay webhook for order ${orderId}, status: ${status}`);
      ...
      console.error("Error processing NabooPay webhook:", error);
